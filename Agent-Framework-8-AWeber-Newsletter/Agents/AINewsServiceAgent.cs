...
using Agent_Framework_8_AWeber_Newsletter.Services;
using Markdig;

namespace Agent_Framework_8_AWeber_Newsletter.Agents
{
    public class AINewsServiceAgent
    {
        ...
        {
            CurrentStatus = "Fetching jamiemaguire.net blog...";
            DebugLog("");
            DebugLog($"[SearchJamieMaguireBlog] Fetching latest blog post via curl...", ConsoleColor.Cyan);

            var results = new List<NewsItemModel>();

            try
            {
                var apiUrl = "https://jamiemaguire.net/index.php/wp-json/wp/v2/posts?per_page=1";
                ...
                {
                    Console.WriteLine("[SearchJamieMaguireBlog] ERROR: curl returned empty response");
                    return results;
                }
                ...
                if (posts.ValueKind == JsonValueKind.Array)
                {
                    foreach (var post in posts.EnumerateArray())
                    {
                        ...
                        var publishedDate = FormatPublishedDate(dateStr);

                        var excerpt = post.GetProperty("excerpt").GetProperty("rendered").GetString() ?? "";
                        excerpt = Regex.Replace(excerpt, "<[^>]+>", "").Trim();
                        if (excerpt.Length > 300)
                            excerpt = excerpt[..300] + "...";

                        DebugLog($"[SearchJamieMaguireBlog]   Found: {title}");

                        results.Add(new NewsItemModel(
                            Headline: title,
                            Summary: excerpt,
                            Source: "jamiemaguire.net",
                            Url: link,
                            Company: "Jamie Maguire",
                            PublishedDate: publishedDate
                        ));
                    }
                }

                DebugLog($"[SearchJamieMaguireBlog] Completed - returning {results.Count} results", ConsoleColor.Green);
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[SearchJamieMaguireBlog] ERROR: {ex.Message}");
            }
        ...
        [Description("Creates an email newsletter from the gathered news, saves it locally as markdown, and sends it as an AWeber broadcast. " +
                     "Call this after gathering news from all companies.")]
        ...
        {
            CurrentStatus = "Generating newsletter and sending via AWeber...";
            DebugLog("");
            DebugLog($"[CreateAndSendNewsletter] Creating newsletter: {subject}", ConsoleColor.Cyan);

            // 1. Save markdown locally (preserves existing behavior)
            ...
            var fullMarkdown = header + markdownContent;

            File.WriteAllText(filename, fullMarkdown);

            SaveLastRunTime();

            DebugLog($"[CreateAndSendNewsletter] Markdown saved to {filename}", ConsoleColor.Green);
            ...
            // 3. Wrap in email-safe HTML template with inline styles
            var bodyHtml = WrapInEmailTemplate(innerHtml, subject);

            // 4. Create plain text version (the original markdown serves as plain text)
            var bodyText = fullMarkdown;

            // 5. Send via AWeber
            try
            {
                var client = new AWeberClient();

                var broadcast = await client.CreateBroadcastAsync(subject, bodyHtml, bodyText);

                DebugLog($"[CreateAndSendNewsletter] Broadcast created: ID={broadcast.BroadcastId}", ConsoleColor.Green);

                var result = $"Newsletter saved to {filename}. AWeber broadcast created (ID: {broadcast.BroadcastId}).";

                if (scheduleMinutesFromNow > 0 && !string.IsNullOrEmpty(broadcast.SelfLink))
                {
                    var scheduledTime = await client.ScheduleBroadcastAsync(broadcast.SelfLink, scheduleMinutesFromNow);
                    result += $" Scheduled to send at {scheduledTime}.";
                    DebugLog($"[CreateAndSendNewsletter] Scheduled for {scheduledTime}", ConsoleColor.Green);
                }
                else
                {
                    result += " Saved as draft (not scheduled).";
                }
            ...
            {
                DebugLog($"[CreateAndSendNewsletter] AWeber error: {ex.Message}", ConsoleColor.Red);
                return $"Newsletter saved to {filename}. AWeber send FAILED: {ex.Message}";
            }
        }

        private static string WrapInEmailTemplate(string innerHtml, string title)
        {
            return $"""
                    <!DOCTYPE html>
                    <html>
                                    ...
                                    <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="background-color:#ffffff; border-radius:8px; overflow:hidden;">
                                        ...
                                        <tr>
                                            <td style="background-color:#1a1a2e; padding:30px 40px; text-align:center;">
                                                <h1 style="color:#ffffff; margin:0; font-size:24px; font-weight:bold;">{System.Net.WebUtility.HtmlEncode(title)}</h1>
                                                <p style="color:#a0a0c0; margin:10px 0 0 0; font-size:14px;">Your Weekly AI News Digest</p>
                                            </td>
                                        </tr>
                                        <!-- Body -->
                                        <tr>
                                        ...
                                        <!-- Footer -->
                                        <tr>
                                            <td style="background-color:#f8f8f8; padding:20px 40px; text-align:center; font-size:12px; color:#999999;">
                                                <p style="margin:0;">Generated by AI News Digest Agent</p>
                                            </td>
                                        </tr>
                                    </table>
                                </td>
